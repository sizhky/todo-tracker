...
def database_active():
    ...
    if not symlink_path.exists():
        typer.echo(
            f"Error: The active database link '{symlink_path}' does not exist.",
            err=True,
        )
    ...
    if not symlink_path.is_symlink():
        typer.echo(
            f"Error: '{symlink_path}' is not a symbolic link. It should be.", err=True
        )
        typer.echo(
            f"The application expects '{ACTIVE_DB_LINK_PATH}' to be a symlink to an actual database file."
        )
    ...
    if not target_path.exists():
        typer.echo(
            f"Warning: The target file '{target_path}' does not currently exist. It may be created on next use.",
            err=True,
        )
    return target_path
...
def _all_dbs():
    ...
    for item in DB_DIR.iterdir():
        if (
            item.is_file()
            and item.name.endswith(".db")
            and item.name != ACTIVE_DB_LINK_FILENAME
        ):
            all_dbs.append(item)
...
def list_databases():
    ...
    if symlink_path.exists() and symlink_path.is_symlink():
        try:
            ...
            typer.echo(
                f"Warning: Active database link '{symlink_path}' is broken.", err=True
            )
            try:
                # Show what it was supposed to point to
                broken_target_name = Path(os.readlink(symlink_path)).name
                typer.echo(
                    f"         It was pointing to '{broken_target_name}'.", err=True
                )
            except OSError:
                pass  # Could not read link
        except OSError:
            typer.echo(
                f"Warning: Could not fully resolve symlink '{symlink_path}'.", err=True
            )
    ...
    for item in sorted(DB_DIR.iterdir()):  # Sort for consistent listing
        ...
        if (
            ...
            if (
                is_symlink_valid
                and current_target_resolved_path
                and item.resolve() == current_target_resolved_path
            ):
                marker = " (active)"
    ...
    if not symlink_path.exists():
        ...
        typer.echo(
            f"\nWarning: '{ACTIVE_DB_LINK_FILENAME}' exists but is not a symlink. Cannot determine active DB via symlink.",
            err=True,
        )
        ...
        not is_symlink_valid and symlink_path.exists() and symlink_path.is_symlink()
    ):  # Broken link case
...
def set_database(
    name: Annotated[
        str,
        typer.Argument(
            help="Name of the database to activate (e.g., 'dev'). '.db' will be appended.",
            autocompletion=_all_dbs,
        ),
    ],
):
    """
    ...
    if ".db" in name.lower():
        typer.echo(
            "Error: Please provide the database name without the '.db' extension. It will be added automatically.",
            err=True,
        )
        raise typer.Exit(code=1)
    ...
    if symlink_path.exists():
        if (
            symlink_path.is_symlink() or symlink_path.is_file()
        ):  # Allow overwriting a regular file too
            try:
                symlink_path.unlink()
            except OSError as e:
            ...
            typer.echo(
                f"Error: '{symlink_path}' exists and is not a file or symlink (e.g., it's a directory). Please remove it manually.",
                err=True,
            )
            raise typer.Exit(code=1)
    ...
    try:
        ...
        if "td.v2" in sys.modules:
            importlib.reload(sys.modules["td.v2"])

        typer.echo(
            f"Success! Active database link '{symlink_path.name}' now points to '{target_db_filename}'."
        )
        typer.echo(
            f"The application will use '{target_db_full_path}' for all subsequent operations."
        )
        ...
        # change db permission to read and write
        os.chmod(target_db_full_path, 0o644)

    except OSError as e:
        typer.echo(
            f"Error creating symlink '{symlink_path}' to '{target_db_filename}': {e}",
            err=True,
        )
        typer.echo(
            "Make sure you have permissions to create symlinks in this directory and that the target name is valid."
        )
        raise typer.Exit(code=1)
...
def remove_database(
    name: Annotated[
        str,
        typer.Argument(
            help="Name of the database to remove (e.g., 'dev'). '.db' will be appended.",
            autocompletion=_all_dbs,
        ),
    ],
):
    ...
    if stem(name) == stem(database_active()):
        ...
        typer.echo(
            f"Error: Cannot remove the active database. Please set another database from ({other_dbs}) as active first.",
            err=True,
        )
        raise typer.Exit(code=1)
